...
const TESTIMONIALS = [
  ...
  { quote: "Every day felt purposeful. You're not just helping—you're becoming part of something bigger.", name: "Vincent", nationality: "Germany", days: 26 }
];

// Parse the mahi exchanger data
const MAHI_DATA: MahiExchanger[] = [
  ...
  { name: 'Natalie Desmarais', nationality: 'Canada', arrivalDate: new Date(2025, 2, 1), departureDate: new Date(2025, 5, 15), totalDays: 106, status: 'departed' },
  { name: 'Luke Halpin', nationality: 'USA', arrivalDate: new Date(2025, 2, 13), departureDate: new Date(2025, 3, 10), totalDays: 28, status: 'departed' },
  { name: 'Pia', nationality: 'Chile', arrivalDate: new Date(2025, 4, 2), departureDate: new Date(2025, 5, 15), totalDays: 44, status: 'departed' },
  { name: 'Tom Brand', nationality: 'USA', arrivalDate: new Date(2025, 4, 15), departureDate: new Date(2025, 6, 6), totalDays: 52, status: 'departed' },
  ...
  { name: 'Enoch Surendran', nationality: 'NZ', arrivalDate: new Date(2025, 5, 9), departureDate: new Date(2025, 6, 10), totalDays: 31, status: 'departed' },
  { name: 'Rachel Gray', nationality: 'UK', arrivalDate: new Date(2025, 5, 13), departureDate: new Date(2025, 6, 8), totalDays: 25, status: 'departed' },
  { name: 'Raina Carfaro', nationality: 'USA', arrivalDate: new Date(2025, 5, 25), departureDate: new Date(2025, 8, 7), totalDays: 74, status: 'departed' },
  { name: 'Innes Orr', nationality: 'UK', arrivalDate: new Date(2025, 6, 20), departureDate: new Date(2025, 7, 21), totalDays: 32, status: 'departed' },
  { name: 'Charlie', nationality: 'UK', arrivalDate: new Date(2025, 6, 28), departureDate: new Date(2025, 7, 1), totalDays: 4, status: 'departed' },
  { name: 'Jackson Pollard', nationality: 'USA', arrivalDate: new Date(2025, 7, 11), departureDate: new Date(2025, 8, 10), totalDays: 30, status: 'departed' },
  { name: 'Einav Levanon', nationality: 'Israel', arrivalDate: new Date(2025, 7, 4), departureDate: new Date(2025, 7, 30), totalDays: 26, status: 'departed' },
  ...
  { name: 'Emily Potter', nationality: 'Canada', arrivalDate: new Date(2025, 8, 8), departureDate: new Date(2026, 1, 16), totalDays: 161, status: 'current' },
  ...
  { name: 'Vincent Roseler', nationality: 'Germany', arrivalDate: new Date(2025, 9, 30), departureDate: new Date(2025, 10, 25), totalDays: 26, status: 'departed' },
  ...
  { name: 'Emily Poulson', nationality: 'Canada', arrivalDate: new Date(2025, 10, 23), departureDate: new Date(2025, 11, 4), totalDays: 11, status: 'departed' },
  ...
  { name: 'Tom Dorman', nationality: 'NZ', arrivalDate: new Date(2025, 11, 22), departureDate: new Date(2026, 1, 12), totalDays: 52, status: 'current' },
...
const MahiExchange: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const totalExchangers = MAHI_DATA.length;
    ...
    const avgStay = Math.round(totalDays / totalExchangers);
    
    return { totalExchangers, totalDays, nationalities, totalHours, current, avgStay };
  }, []);
  ...
  return (
    <div className="space-y-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
          <div className="space-y-4">
            <h2 className="font-serif text-5xl md:text-7xl tracking-tighter leading-tight">Mahi Exchange</h2>
          ...
          <p className="font-serif text-2xl text-[#555] leading-relaxed italic">
            "A reciprocal exchange of energy — where travellers contribute meaningful work in return for immersion in regenerative land practice."
          </p>
          <div className="text-sm leading-relaxed text-[#666] max-w-lg">
            Our Mahi Exchange programme welcomes people from around the world to live and work at Mangaroa Farms. In return for 20-25 hours of hands-on contribution per week, exchangers receive accommodation, meals, and deep connection to the land and community.
          </div>
          <a 
      ...
      <div className="bg-[#F9F8F6] p-10 md:p-16 rounded-[24px] border border-[#E5E1DD] space-y-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="space-y-2">
            <h4 className="text-[9px] uppercase tracking-widest opacity-30 font-bold block">Total Exchangers</h4>
            <span className="font-serif text-6xl tracking-tighter">{stats.totalExchangers}</span>
          </div>
          <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] uppercase tracking-[0.4em] font-black opacity-30">Exchangers</h3>
            <div className="flex gap-2">
              <button
      ...
      <div className="bg-gradient-to-r from-[#2D4F2D] to-[#1A3A1A] p-12 rounded-3xl text-white text-center space-y-6">
        ...
        <p className="text-white/80 max-w-xl mx-auto">
          Join our community of regenerative practitioners. Minimum stay is 2 weeks, with most exchangers staying 4-8 weeks.
        </p>
        <a 
          href="https://mangaroa.org/mahi-exchange"
          target="_blank"
